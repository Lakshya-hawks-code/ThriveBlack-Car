...
export const GetOrders = async (req, res) => {
  try {
    ...
    if (request?.bookingfilter) {
      if (request.bookingfilter === 'upcoming') {
        filter.transcation_id = { $ne: null };
        filter.$or = [
          { pikup_date: { $gte: currentDate } },
          { returndate: { $gte: currentDate } }
        ];
      } else if (request.bookingfilter === 'leads') {
        filter.transcation_id = "";;
        filter.pikup_date = { $lt: currentDate };
      } else if (request.bookingfilter === 'bookdone') {
        filter.transcation_id = { $ne: null };
        filter.pikup_date = { $lt: currentDate };
      }
    }
    ...
    for (const order of orders) {
      ...
      filteredOrder.pickupTime = order?.time;
...
export const AddCallStatus = async (req, res) => {
  try {
    ...
    if (OrdersData) {
      ...
      if (request?.staff_id) {
        updatefields.staff_id = request.staff_id;
      }
