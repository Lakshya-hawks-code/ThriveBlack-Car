        ...
        if (user.user_type === 2) {
            ...
            if (request?.bookingfilter) {
                if (request?.bookingfilter === 'upcoming') {
                    filter.pikup_date = { $gt: currentDate };
                } else if (request?.bookingfilter === 'completed') {
                    filter.pikup_date = { $lt: currentDate };
                } else if (request?.bookingfilter === 'cancel') {
                    filter.book_status = 2;
                }
            }
            ...
            if (Bookings) {
                let filteredBookings = Bookings.map((booking) => {
                    let filteredOrder = {
                        ...
                        amount: booking?.assignamount ? booking?.assignamount : "",
                    };
                    // Conditionally add passengersDetails based on other_pass_name and other_pass_phone
