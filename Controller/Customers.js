...
import { getUserById, getAllUserDetails } from "../Model/AdminModel.js"
...
export const GetCustomers = async (req, res) => {
  try {
    ...
    let customerData = await getAllUserDetails({ user_type: 3 })

    // Check if customerData is an array, if not, convert it to an array
    ...
    // Add customers with user_type: 3 and calculate total_orders
    customerData.forEach((customer) => {
      const customerOrders = orders.filter(order => order.email === customer.email);
      const totalOrders = customerOrders.length;
      const oldestOrder = customerOrders.sort((a, b) => a.createdAt - b.createdAt)[0];

      if (!(customer.email in emailData)) {
        filteredOrders.push({
          customername: customer?.name,
          email: customer?.email,
          phone: customer?.phone ? customer?.phone : "",
          booking_type: oldestOrder ? (oldestOrder?.type === 'home-tab' ? `By Hours - ${oldestOrder?.hour || ''}` : 'By-Transfer') : "",
          register_date: customer?.createdAt,
          update_date: customer?.updatedAt,
